...
namespace RLGC {
	...
	public:
		constexpr static float MAX_TIME_IN_AIR = 1.75f;

		virtual float GetReward(const Player& player, const GameState& state, bool isFinal) {
			if (player.ballTouchedStep && !player.isOnGround) // touch ball and not on the ground
			{
				float air_time_frac = RS_MIN(player.airTime, MAX_TIME_IN_AIR) / MAX_TIME_IN_AIR;
				float height_frac = state.ball.pos.z / CommonValues::CEILING_Z;
...
// TODO, make a reward to reward the bot for positioning between the ball and the opposing goal